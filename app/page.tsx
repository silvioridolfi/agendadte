...
const avatarColors = ['bg-[#d8e8f2]', 'bg-[#f5dfcf]', 'bg-[#e3e2f5]', 'bg-[#dcebdd]', 'bg-[#f8e8bd]', 'bg-[#f5d9e2]']
...
export default function Page() {
  ...
  return <div className="min-h-screen bg-[#f5f7f8] text-[#21313b]">
    <header className="border-b border-[#dce4e8] bg-white"><div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-4 lg:px-10"><div className="flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-xl bg-[#1e506d] text-white"><ClipboardList /></div><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#638092]">Equipo FED</p><h1 className="text-lg font-bold tracking-tight">Agenda Territorial</h1></div></div><div className="flex items-center gap-3"><Button variant="ghost" size="sm" className="text-[#577080]" onClick={() => setSection(section === 'agenda' ? 'board' : 'agenda')}><LayoutDashboard data-icon="inline-start" /><span className="hidden sm:inline">{section === 'agenda' ? 'Tablero coordinador' : 'Mi agenda'}</span></Button><Avatar className="size-9"><AvatarFallback className={`${fedColor(feds ?? [], profile.id)} text-xs font-bold text-[#274c60]`}>{initials(profile.nombre_completo)}</AvatarFallback></Avatar><div className="hidden text-right sm:block"><p className="text-xs font-semibold">{profile.nombre_completo}</p><p className="text-[11px] text-muted-foreground">FED · {profile.distritos_a_cargo.join(', ') || 'Sin distritos'}</p></div><Button variant="ghost" size="icon" aria-label="Cambiar perfil" onClick={() => { setProfile(null); setSection('agenda') }}><ChevronDown /></Button></div></div></header>
    {section === 'agenda'
...
function ProfileSelect({ feds, error, onSelect }: { feds: Fed[] | null, error: string, onSelect: (fed: Fed) => void }) {
  return <main className="flex min-h-screen items-center justify-center bg-[#eef3f5] px-5 py-10"><div className="w-full max-w-3xl"><div className="mb-10 flex items-center gap-3"><div className="flex size-11 items-center justify-center rounded-xl bg-[#1e506d] text-white"><ClipboardList /></div><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#638092]">Equipo FED</p><h1 className="text-xl font-bold">Agenda Territorial</h1></div></div><div className="mb-8"><p className="mb-2 text-sm font-semibold text-[#638092]">Bienvenido/a</p><h2 className="text-3xl font-bold tracking-tight text-[#21313b]">¿Con quién vas a trabajar hoy?</h2><p className="mt-2 text-muted-foreground">Seleccioná tu perfil para entrar a la agenda.</p></div>
    {error ? <ErrorBox message={`No se pudieron cargar los FEDs: ${error}`} /> : !feds ? <Loading /> : !feds.length
      ? <p className="rounded-2xl border border-dashed border-[#bdd0d9] p-8 text-center text-sm text-[#577080]">Todavía no hay FEDs cargados en la tabla <code>feds</code>.</p>
      : <div className="grid gap-4 sm:grid-cols-2">{feds.map((fed, i) => <button key={fed.id} onClick={() => onSelect(fed)} className="group flex items-center gap-4 rounded-2xl border border-[#d7e1e6] bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#79a1b5] hover:shadow-md"><Avatar className="size-12"><AvatarFallback className={`${avatarColors[i % avatarColors.length]} font-bold text-[#274c60]`}>{initials(fed.nombre_completo)}</AvatarFallback></Avatar><div className="flex-1"><p className="font-semibold">{fed.nombre_completo}</p><p className="mt-1 text-sm text-muted-foreground">{fed.distritos_a_cargo.length ? `Distritos: ${fed.distritos_a_cargo.join(', ')}` : 'Sin distritos asignados'}</p></div><ChevronRight className="text-[#aac0ca] transition group-hover:translate-x-1" /></button>)}</div>}
  </div></main>
}
...
function AgendaView({ fed, reloadKey, onNew, onSelect }: { fed: Fed, reloadKey: number, onNew: () => void, onSelect: (item: AgendaItem) => void }) {
  ...
  return <main className="mx-auto max-w-[1440px] px-5 py-7 lg:px-10"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-[#638092]">Mi agenda</p><h2 className="mt-1 text-2xl font-bold tracking-tight sm:text-3xl">Semana del {fmt(weekStart, { day: 'numeric', month: 'long' })} al {fmt(weekEnd, { day: 'numeric', month: 'long' })}</h2><p className="mt-2 text-sm text-muted-foreground">{fed.nombre_completo.split(' ')[0]}, acá está tu territorio.</p></div><div className="flex items-center gap-2"><Button variant="outline" size="icon" aria-label="Semana anterior" onClick={() => setWeekStart(addDays(weekStart, -7))}><ChevronLeft /></Button><Button variant="outline" size="sm" onClick={() => setWeekStart(startOfWeek(new Date()))}>Hoy</Button><Button variant="outline" size="icon" aria-label="Semana siguiente" onClick={() => setWeekStart(addDays(weekStart, 7))}><ChevronRight /></Button><Button onClick={onNew} className="hidden bg-[#1e506d] hover:bg-[#173d53] sm:flex"><Plus data-icon="inline-start" />Nueva acción</Button></div></div>
    <div className="mt-8">{error ? <ErrorBox message={error} /> : !items ? <Loading /> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{items.map(item => { const d = parse(item.fecha); return <button key={item.id} onClick={() => onSelect(item)} className="text-left"><Card className={`h-full border-[#dce4e8] shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${item.fecha === today ? 'ring-2 ring-[#9ec1d0] ring-offset-2' : ''}`}><CardHeader className="flex-row items-start justify-between pb-3"><div><p className="text-xs font-bold uppercase tracking-wider text-[#638092]">{fmt(d, { weekday: 'short' }).replace('.', '')}</p><p className="mt-1 text-3xl font-bold text-[#21313b]">{d.getDate()}</p></div><StatusBadge status={item.estado} /></CardHeader><CardContent><div className="flex flex-col gap-3"><p className="text-xs font-medium text-[#638092]">{timeRange(item)}</p><p className="line-clamp-3 text-sm font-semibold leading-snug">{item.school?.distrito ? `${item.school.distrito} | ` : ''}{schoolLabel(item.school)}</p><ActionChip label={item.accion} />{item.sub_accion && <p className="text-xs text-muted-foreground">{item.sub_accion}</p>}</div></CardContent></Card></button> })}<Card className="border-dashed border-[#bdd0d9] bg-transparent shadow-none md:col-span-2 xl:col-span-4"><CardContent className="flex min-h-28 flex-col items-center justify-center gap-2 text-center"><CalendarDays className="text-[#87a5b3]" /><p className="text-sm font-medium text-[#577080]">{items.length ? 'No hay más acciones esta semana' : 'No hay acciones cargadas esta semana'}</p><Button variant="link" className="text-[#1e506d]" onClick={onNew}>Agregar una acción</Button></CardContent></Card></div>}</div>
    <Button onClick={onNew} size="icon" aria-label="Nueva acción" className="fixed bottom-6 right-5 size-14 rounded-full bg-[#1e506d] shadow-lg hover:bg-[#173d53] sm:hidden"><Plus /></Button></main>
}
...
function CoordinatorView({ feds, reloadKey, onSelect }: { feds: Fed[], reloadKey: number, onSelect: (item: AgendaItem) => void }) {
  ...
  return <main className="mx-auto max-w-[1440px] px-5 py-7 lg:px-10"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-[#638092]">Tablero del coordinador</p><h2 className="mt-1 text-3xl font-bold tracking-tight">Seguimiento territorial</h2><p className="mt-2 text-sm capitalize text-muted-foreground">{title} · {items ? `${filtered.length} acciones` : '…'}</p></div><div className="flex flex-wrap items-center gap-2"><Button variant="outline" size="icon" aria-label="Anterior" onClick={() => setAnchor(shift(anchor, range, -1))}><ChevronLeft /></Button><div className="flex rounded-lg border border-[#dce4e8] bg-white p-0.5">{([['day', 'Día'], ['week', 'Semana'], ['month', 'Mes']] as const).map(([v, l]) => <button key={v} onClick={() => setRange(v)} className={`rounded-md px-3 py-1 text-sm ${range === v ? 'bg-[#1e506d] text-white' : 'text-[#577080]'}`}>{l}</button>)}</div><Button variant="outline" size="icon" aria-label="Siguiente" onClick={() => setAnchor(shift(anchor, range, 1))}><ChevronRight /></Button><Button variant="outline" size="sm" onClick={() => setAnchor(new Date())}>Hoy</Button></div></div>
    <div className="mt-7 flex flex-col gap-3 rounded-xl border border-[#dce4e8] bg-white p-3 shadow-sm md:flex-row md:flex-wrap"><div className="relative min-w-52 flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#8aa1ad]" /><Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por escuela, CUE o FED..." className="border-0 bg-[#f5f7f8] pl-10 shadow-none" /></div>
    ...
    <div className="mt-8 flex flex-col gap-6">{error ? <ErrorBox message={error} /> : !items ? <Loading /> : !groups.length ? <p className="rounded-xl border border-dashed border-[#bdd0d9] p-8 text-center text-sm text-[#577080]">No hay acciones para este período y filtros.</p> : groups.map(([id, group]) => <section key={id}><div className="mb-3 flex items-center gap-3"><Avatar className="size-9"><AvatarFallback className={`${fedColor(feds, id)} text-xs font-bold text-[#274c60]`}>{initials(fedName(id))}</AvatarFallback></Avatar><div><h3 className="text-sm font-bold">{fedName(id)}</h3><p className="text-xs text-muted-foreground">{group.length} {group.length === 1 ? 'acción' : 'acciones'}</p></div></div><div className="divide-y divide-[#e4eaed] overflow-hidden rounded-xl border border-[#dce4e8] bg-white shadow-sm">{group.map(item => <button key={item.id} className="flex w-full flex-col gap-3 p-4 text-left transition hover:bg-[#f8fafb] sm:flex-row sm:items-center" onClick={() => onSelect(item)}><span className="w-28 text-sm font-semibold text-[#638092]">{range !== 'day' && <span className="block text-xs capitalize">{fmt(parse(item.fecha), { weekday: 'short', day: 'numeric', month: 'short' })}</span>}{item.hora_inicio ? hhmm(item.hora_inicio) : '—'}</span><div className="flex-1"><p className="font-semibold">{schoolLabel(item.school)}</p><p className="mt-1 text-xs text-muted-foreground">{item.school?.distrito ? `Distrito ${item.school.distrito}` : ''}{item.sub_accion ? `${item.school?.distrito ? ' · ' : ''}${item.sub_accion}` : ''}</p></div><ActionChip label={item.accion} /><StatusBadge status={item.estado} /></button>)}</div></section>)}</div></main>
}
...
function ItemForm({ fed, item, onCancel, onSaved }: { fed: Fed, item: AgendaItem | null, onCancel: () => void, onSaved: () => void }) {
  ...
  return <form onSubmit={submit} className="flex flex-col gap-5"><div className="grid gap-4 sm:grid-cols-2"><label className="flex flex-col gap-2 text-sm font-semibold">Fecha<Input type="date" required value={form.fecha} onChange={e => set('fecha', e.target.value)} /></label><div className="flex flex-col gap-2 text-sm font-semibold">Escuela (opcional)<SchoolPicker value={school} onChange={setSchool} /></div></div>
    <div className="flex flex-col gap-2"><p className="text-sm font-semibold">Tipo de acción</p><div className="flex flex-wrap gap-2">{ACCIONES.map(name => <button key={name} type="button" onClick={() => set('accion', name)} className={`rounded-md px-2.5 py-1.5 text-left text-[10px] font-bold transition ${actionStyle[name]} ${form.accion === name ? 'ring-2 ring-[#1e506d] ring-offset-1' : 'opacity-70 hover:opacity-100'}`}>{name}</button>)}</div></div>
    ...
    <div className="flex justify-end gap-2 border-t border-[#e4eaed] pt-4"><Button variant="outline" type="button" onClick={onCancel}>Cancelar</Button><Button type="submit" disabled={saving} className="bg-[#1e506d] hover:bg-[#173d53]">{saving && <Loader2 className="animate-spin" data-icon="inline-start" />}Guardar acción</Button></div></form>
}
